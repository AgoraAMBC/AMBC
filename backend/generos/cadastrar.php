...
try {
    ...
    $stmt = $pdo->prepare('INSERT INTO genero (descricao) VALUES (:descricao) RETURNING id_genero AS id, descricao');
    $stmt->execute([':descricao' => $descricao]);
    $row = $stmt->fetch();

    jsonResposta($row ?: ['mensagem' => 'Gênero cadastrado com sucesso'], 201);
